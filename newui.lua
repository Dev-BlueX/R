...
S:AddDropdown("Select Stats", Stats, function(value)
    _G.Stats = value
...
-- Hàm lấy danh sách mục tiêu gần
    ...
    for _, enemy in pairs(workspace.Enemies:GetChildren()) do
        if enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
            ...
            if distance <= radius then
                table.insert(targets, enemy.HumanoidRootPart)
            end
        end
    end
    return targets
...
-- Biến để bật/tắt tự động tấn công
...
spawn(function()
    while true do
        if autoAttackEnabled then
            local targets = getTargets(200) -- Phạm vi tấn công
            if #targets > 0 then
                -- Gửi sự kiện tấn công
                registerAttack:FireServer(0.5)
                for _, target in ipairs(targets) do
                    -- Gửi sự kiện đánh mục tiêu
                    registerHit:FireServer(target)
                end
            end
        end
        wait(0.1) -- Tốc độ vòng lặp
    end
end)
