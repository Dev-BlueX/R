...
Notify("Script Update 28/1/2025", "[+] New Ui", 5)
...
-- Biến để theo dõi trạng thái của menu
        local isMenuOpen = false  

        MODILEGUISOMEXHUB.MouseButton1Click:Connect(function()
    -- Lật lại trạng thái menu
    isMenuOpen = not isMenuOpen
    
    -- Tìm đối tượng POWERXHUB và thay đổi trạng thái
        local powerXHub = game.CoreGui:FindFirstChild("POWERXHUB")
    if powerXHub then
        powerXHub.Enabled = not powerXHub.Enabled
    end
    
    -- Cập nhật hình ảnh của MODILEMAGE dựa trên trạng thái menu
    if isMenuOpen then
        MODILEMAGE.Image = "http://www.roblox.com/asset/?id=111751700463789"  -- Hình ảnh khi menu bật
    else
        MODILEMAGE.Image = "http://www.roblox.com/asset/?id=137924388649619"  -- Hình ảnh khi menu tắt
    end
end)
        ...
        MODILEMAGE.Image = "http://www.roblox.com/asset/?id=137924388649619"  -- Hình ảnh mặc định (khi menu tắt)
do
...
-- Window
local Library = library:AddWindow("BLUEX","BloxFruits",Enum.KeyCode.RightControl)
...
-- Local Player Tab
L:AddSeperator("Server")

L:AddTextbox("Job ID", true, function(value)
	_G.Job = value
	end)

L:AddButton("Join Server Job ID",function()
...
L:AddSeperator("Teleport Special Island")
...
L:AddDropdown("Select Stats", Stats, function(value)
    _G.Stats = value
    -- Cập nhật giá trị của _G.Stats tùy theo lựa chọn
    if value == "Melee" then
        melee = true
...
S:AddToggle("Fast Attack", _G.Settings.Attack, function(value)
    ...
    SaveSettings()  -- Lưu lại cài đặt
end)

-- Hàm tìm kiếm mục tiêu trong phạm vi tấn công
...
-- Hàm tấn công không có cooldown
function AttackNoCoolDown()
    ...
    -- Gửi lệnh tấn công và hit event
    pcall(function()
        ...
        if #targets > 0 then
            ...
            -- Gửi lệnh hit event với các mục tiêu đã được tìm
            hitEvent:FireServer(primaryPart, targets)
        else
            task.wait(0.1)  -- Thêm một chút thời gian chờ để giảm tải tài nguyên
        end
    end)
end

-- Cờ bật tấn công nhanh
FastAttack = _G.FastAttack or false  -- Sử dụng giá trị mặc định nếu chưa có giá trị cũ

spawn(function()
    while wait(0.1) do
        if FastAttack then
            AttackNoCoolDown()  -- Thực hiện tấn công nhanh
        end
    end
end)
...
V:AddToggle("Auto Upgrade Race V2", _G.Settings.v2race, function(value)
    _G.Settings.v2race = value
    SaveSettings()
end)
                            ...
                            if game:GetService("Workspace").Enemies:FindFirstChild("Zombie") then
                                for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                    ...
                                    FastMon = v.HumanoidRootPart.CFrame
                                        until game.Players.LocalPlayer.Backpack:FindFirstChild("Flower 3") or not v.Parent or v.Humanoid.Health <= 0 or _G.Settings.v2race == false
                                        StardMag = false
                                    end
                                end
                            else
