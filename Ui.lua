  ...
  DropShadowHolder.Position = UDim2.new(0, (HirimiGui.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0, (HirimiGui.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
	...
	function Tabs:CreateTab(TabConfig)
		...
		function Sections:AddSection(Title)
			...
			function Items:AddDropdown(DropdownConfig)
                ...
                SearchFrame.Parent = ScrollSelect
    ...
    -- clear cũ
                    ...
                    for _, option in ipairs(DropdownConfig.Options) do
                        if filterText == "" or string.find(string.lower(option), string.lower(filterText)) then
                            ...
                            OptionButton.Parent = ScrollSelect

                            OptionButton.MouseButton1Click:Connect(function()
                                DropdownFunc.Value = option
                                OptionSelecting.Text = option
                ...
                SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
                    ...
                    for _, child in ipairs(ScrollSelect:GetChildren()) do
                        if child:IsA("Frame") and child.Name == "Option" then
                            local optionTextLabel = child:FindFirstChild("OptionText")
                            if optionTextLabel then
                                local optionText = string.lower(optionTextLabel.Text)
                                if filterText == "" or string.find(optionText, filterText) then
                                    child.Visible = true
                                else
                                    child.Visible = false
                                end
    ...
    -- Cập nhật lại CanvasSize (chỉ tính các Option còn visible)
                    task.wait()
                    local totalHeight = 0
                    for _, child in ipairs(ScrollSelect:GetChildren()) do
                        if child:IsA("Frame") and child.Name == "Option" and child.Visible then
                            totalHeight = totalHeight + child.Size.Y.Offset + 3 -- 3 là padding
                        end
                    end
                    ScrollSelect.CanvasSize = UDim2.new(0, 0, 0, totalHeight + SearchFrame.Size.Y.Offset + 10)
                end)
